...
def umap_projections(transformed_df, metadata_df, N_COMP, metric, min_dist):
    ...
    results = transformed_df[transformed_df.columns.drop(list(transformed_df.filter(regex='feature_')))]
    results = results.drop(['duration'], axis=1)
    umap_df = results 

    # Merge metadata df and results indices
    umap_df = umap_df.set_index('recording_id').join(metadata_df.set_index('recording_id'))
