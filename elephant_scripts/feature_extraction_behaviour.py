...
def apply_bandpass_filter(wav, low_freq, high_freq):
    ...
    sos = scipy.signal.butter(5, [low_normalized, high_normalized], btype='band', output='sos')
...
def wav_cookiecutter(path, annotation, window_size, position, low_freq, high_freq):
    # Read the audio
...
def feature_extraction(df):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Processing"):
        ...
        sr = 4000
        window_size = 4
        low_freq = row.low_freq  # Extract low frequency value from the DataFrame
        high_freq = row.high_freq  # Extract high frequency value from the DataFrame

        # Apply the wav_cookiecutter function combining all of the audio pre-processing steps
        wav = wav_cookiecutter(path, annotation, window_size, 'middle', low_freq, high_freq)
