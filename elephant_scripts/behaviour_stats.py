"""
Functions to analyse the relationship between the n=1 reduced acoustic feature dimension (UMAP1) and our fixed and random effects of interest, behavioural context, distress, age and sex. These include plotting boxplots to visualise the relationships, testing for collinearity and zero variance as well as specifying and running the mixed effects model.
...
def create_boxplots(umap_df):
    ...
    ax1 = axes[0, 0]
    sns.boxplot(data=filtered_df, x='Final_Category', y='UMAP1', width=0.6, showfliers=True, ax=ax1)
    add_mean_line(filtered_df, 'UMAP1', ax1)  # Add mean line to the plot
    ax1.set_title('a) Vocalisation~Behaviour', fontsize=16, loc='left')  # Adjust title font size and alignment
    ...
    ax2.set_title('b) Vocalisation~Age', fontsize=16, loc='left')  # Adjust title font size and alignment

    # Plot 3
    filtered_df_s = umap_df[~umap_df['Sex'].isin(['Unknown'])]
    ax3 = axes[1, 0]
    sns.boxplot(data=filtered_df_s, x='Sex', y='UMAP1', width=0.6, showfliers=True, ax=ax3)
    add_mean_line(filtered_df_s, 'UMAP1', ax3)  # Add mean line to the plot
    ax3.set_title('c) Vocalisation~Sex', fontsize=16, loc='left')  # Adjust title font size and alignment
    ...
    ax4.set_title('d) Vocalisation~Distress', fontsize=16, loc='left')  # Adjust title font size and alignment
...
def analyze_model_performance(df, behaviour_model_result):
    ...
    sns.lmplot(x="Predicted_UMAP", y="residuals", data=behaviour_model_performance)
...
def perform_normality_tests(data):
    ...
    shapiro_result = "Data is normally distributed." if shapiro_p_value > 0.05 else "Data is not normally distributed."

    # Anderson-Darling test
    anderson_test_stat, anderson_critical_values, anderson_p_values = stats.anderson(data, dist='norm')
    anderson_p_value = max(anderson_p_values)
    anderson_result = "Data is normally distributed." if anderson_p_value > 0.05 else "Data is not normally distributed."

    # Jarque-Bera test
    jb_test_stat, jb_p_value = stats.jarque_bera(data)
    jb_result = "Data is normally distributed." if jb_p_value > 0.05 else "Data is not normally distributed."

    # Durbin-Watson test
    dw_statistic = durbin_watson(data)
    dw_result = "Data is normally distributed." if (1.5 <= dw_statistic <= 2.5) else "Data is not normally distributed."

    return {
        'Skewness': {'Score': skewness, 'Result': "Data is normally distributed." if (-1 < skewness < 1) else "Data is not normally distributed."},
        'Kurtosis': {'Score': kurtosis, 'Result': "Data is normally distributed." if (-1 < kurtosis < 1) else "Data is not normally distributed."},
        'Shapiro-Wilk': {'Test Statistic': shapiro_test_stat, 'p-value': shapiro_p_value, 'Result': shapiro_result},
        'Anderson-Darling': {'Test Statistic': anderson_test_stat, 'p-value': anderson_p_value, 'Result': anderson_result},
        'Jarque-Bera': {'Test Statistic': jb_test_stat, 'p-value': jb_p_value, 'Result': jb_result},
